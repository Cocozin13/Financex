...
def buy():
    ...
    if (request.method == "POST"):
        ...
        if not symbol:
            return apology("Stock name is required")
        if not item:
        ...
        try:
            ...
            return apology("Share needs to be a number")

        if (shares <= 0):
            return apology("Number needs to be positive")
        ...
        if cash < total_price:
            return apology("Insufficent money")
        else:
...
def quote():
    ...
    if (request.method == "POST"):
        ...
        symbol = request.form.get("symbol").upper()
        if not symbol:
            return apology("The stock name is required")
...
def register():
    ...
    if (request.method == "POST"):
        ...
        # check if name is blank or alredy exists
        if not username:
    ...
    # check if password is the same and not blank
        ...
        if password != confirmation:
            return apology("The passwords need to match")
        if not specialChars.intersection(password):
            return apology("The password needs to have at least one special character")
    # Insert user into db and hash the password and keep the hash in the database
